 ...
 *
 * Derived from plannodes.h of PostgreSQL 9.4.2.
 *
...
typedef struct PlannedStmt
{
	...
	bool		hasOnline;		/* has online join? */

	uint32		withTime;		/* time limit of online join in ms; 0 if not available */	

	double		confidence;		/* confidence of online join */

	uint32		reportInterval;	/* report interval in ms of online sample join */

	uint32		nInitSamples;	/* number of initial samples for plan selection */
...
/* Online sample join plan. 
 *
 * The join condition of the first i relations must in the form of 
 * (<cols of ith relation> <op> <cols of first i-1 relations>)
 * where op can be <, <=, =, >, >=.
 *
 * Btree index on the columns on the left side of the join conditions must exist.
 *
 * The left args of join_quals have varno set to 0 and varattno set to
 * the attno in the index relation if there's a valid index on the left arg.
 * The old varno and varattno are in the varnoold and varoattno fields.
 *
 * target[i]->resno is the no. of var
 * target[i]->resorigtbl, resorigcol are the relation and column index of 
 * the variable
 * target[i]->expr is always NULL
 *
 * The arrays all start from 1 (with a wasted cell at 0).
 *
 * */
typedef struct OnlineSampleJoin {
	Plan		plan;
	
	uint32		no;				/* no. of this plan */
	uint32		nrel;			/* the number of relations to join */

	Oid			*relid;			/* the array of the oids of the relations */
	Oid			*indexid;		/* the array of the oids of the btree indecies for join quals */
	Oid			*primary_indexid;	/* the array of the oids of the primary indecies */
...
typedef struct OnlineAggSum {
	Expr			info;		/* Expr header */
	
	Expr			*arg;
	Oid				arg_typeid;
} OnlineAggSum;

typedef struct OnlineAggCount {
	Expr			info;		/* Expr header */
	
	Const			*scale;		/* result = count(*) * (scale ? 1 : scale) */
} OnlineAggCount;
