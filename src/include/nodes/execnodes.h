 ...
 * 
 * Derived from execnodes.h of PostgreSQL 9.4.2.
 *
...
typedef struct ExprContext
{
	...
	/* for online sample join */
	TupleTableSlot **ecxt_reltuples;	/* array of tuples sampled from each relation */
...
/** 
 * Online sample join and aggregation state nodes
 */

typedef struct OnlineAggState {
	PlanState	ps;				/* to include the first field NodeTag */
								/* have to set up ps_ResultTupleSlot to return results*/	

	int			naggs;
	List		*peraggs;		/* OnlineAgg_NumericAggPerAggState in target_states,
									for the convenient impl. of agg funcs */
	List		*target_states; /* OnlineAgg_NumericAggPerAggState or 
								   ExprState */

	uint32		nInitSamples;		/* number of initial samples for plan selection.
										if nInitSamples is 0, an arbitrary plan will
										be selected. */
	...
	struct OnlineSampleJoinState * join_state;
	...
	TimestampTz timeout_time;
	TimestampTz now_time; */
	
	bool hasGrouping;

	/* these fields are used when there's group by clause */
	FmgrInfo   *eqfunctions;	/* per-grouping-field equality fns */
	FmgrInfo   *hashfunctions;	/* per-grouping-field hash fns */
	TupleHashTable hashtable;	/* hash table with one entry per group */
	TupleTableSlot *hashslot;	/* slot for loading hash table */
	bool		table_filled;	/* hash table filled yet? (reset when iter done) */
	TupleHashIterator hashiter; /* for iterating through hash table */
...
typedef struct OnlineSampleJoinState {
	...
	uint32			ntargets;
	TupleTableSlot	*outputs;	/* outputs of the tlist */
	List			*output_expr_states;

	uint32			nrels;
	TupleTableSlot	**tuples;	/* tuples */
	Relation		*heaprels;
	Relation		*join_index;
	int				*n_joinkeys;
	ScanKey			*join_keys;
