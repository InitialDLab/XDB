 ...
 *
 * Derived from gram.y of PostgreSQL 9.4.2
 *
 * IDENTIFICATION
...
%type <ival>	withtime_clause confidence_clause reportinterval_clause 
				initsample_clause
...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
  ...
  ALSO ALTER ALWAYS ANALYSE ANALYZE AND ANY ARRAY AS ASC
	...
	COMMITTED CONCURRENTLY CONFIDENCE CONFIGURATION CONNECTION CONSTRAINT CONSTRAINTS
	...
	INCLUDING INCREMENT INDEX INDEXES INHERIT INHERITS INITIALLY INITSAMPLE INLINE_P
	...
	OBJECT_P OF OFF OFFSET OIDS ON ONLINE ONLY OPERATOR OPTION OPTIONS OR
	...
	RELATIVE_P RELEASE RENAME REPEATABLE REPLACE REPLICA REPORTINTERVAL
	RESET RESTART RESTRICT RETURNING RETURNS REVOKE RIGHT ROLE ROLLBACK
	ROW ROWS RULE

	SAVEPOINT SCAN SCHEMA SCROLL SEARCH SECOND_P SECURITY SELECT SEQUENCE SEQUENCES
	...
	WHEN WHERE WHITESPACE_P WINDOW WITH WITHIN WITHOUT WITHTIME WORK WRAPPER WRITE
 ...
 *
 *****************************************************************************/
ScanStmt: SCAN qualified_name
      {
        ScanStmt *s = makeNode(ScanStmt);
        s->relation = $2;
        $$ = (Node *) s;
      }
...
/*
 *WITHTIME Definitions
 */
withtime_clause: WITHTIME Iconst					{ $$ = $2;}
			| /*EMPTY*/								{ $$ = 0; /* defaults no limit */ }
			;
confidence_clause:   CONFIDENCE Iconst				{ $$ = $2; }
			| /*EMPTY*/								{ $$ = 100; /* defaults to 100% */ }
			;

reportinterval_clause: REPORTINTERVAL Iconst		{ $$ = $2; }
					 | /*EMPTY*/					{ $$ = 1; /* defaults to 1s */ }
					 ;

initsample_clause: INITSAMPLE Iconst				{ $$ = $2; }
				|	/* EMPTY */						{ $$ = 0; /* defaults to 0 */}
				;
