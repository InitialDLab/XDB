			 ...
			 bool is_internal)
{
	...
	if (IsBootstrapProcessingMode())
	{
		ereport(DEBUG1, (errmsg("index_register(heapRelationId = %u, indexRelationId = %u, indexInfo = %lu), name = %s", 
						heapRelationId, indexRelationId, (uint64) indexInfo, indexRelationName)));
