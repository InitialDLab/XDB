...
static void
do_numericvar_accum(NumericAggState *state, NumericVar *X)
{
	NumericVar	X2;
	MemoryContext old_context;
 ...
 */
static void
uint8_to_numericvar(uint64 uval, NumericVar *var)
{
	uint64 newuval;
	NumericDigit *ptr;
	int			ndigits;

	/* uint8 can require at most 20 decimal digits; add one for safety */
	alloc_var(var, 21 / DEC_DIGITS);
...
static void
double_to_numericvar(double val, NumericVar *var) {
	char		buf[DBL_DIG + 100];
	...
	/* Assume we need not worry about leading/trailing spaces */
	(void) set_var_from_str(buf, buf, var);
}
...
/* onlineagg_numeric.c have to be compiled with numeric.c since we are using
 * the static functions here */
#define ONLINEAGG_NUMERIC_C_INCLUDED_FROM_NUMERIC_C
#include "onlineagg_numeric.c"
