...
ExprState *
online_ExecInitExpr(Expr *node, PlanState * parent, ExprStateEvalFunc scalarVarEvalFunc) {
	ExprState *state;
	...
	switch (nodeTag(node)) {
		case T_Var:
			{
				if (((Var *) node)->varattno == InvalidAttrNumber) {
					elog(ERROR, "invalid attnumber in var node");
				}
				state = (ExprState *) makeNode(ExprState);
				state->evalfunc = scalarVarEvalFunc;
			}
			break;
		...
		case T_FuncExpr:
			{
				...
				fstate->xprstate.evalfunc = (ExprStateEvalFunc) ExecEvalFunc;
				fstate->args = (List *)
					online_ExecInitExpr((Expr *) funcexpr->args, parent, scalarVarEvalFunc);
				fstate->func.fn_oid = InvalidOid;		/* not initialized */
				state = (ExprState *) fstate;
		...
		case T_OpExpr:
			{
				...
				fstate->xprstate.evalfunc = (ExprStateEvalFunc) ExecEvalOper;
				fstate->args = (List *)
					online_ExecInitExpr((Expr *) opexpr->args, parent, scalarVarEvalFunc);
				fstate->func.fn_oid = InvalidOid;		/* not initialized */
				state = (ExprState *) fstate;
			}
			break;
		...
		case T_BoolExpr:
			{
				...
				bstate->args = (List *)
					online_ExecInitExpr((Expr *) boolexpr->args, parent, scalarVarEvalFunc);
				state = (ExprState *) bstate;
			}
			break;
		case T_List:
			{
				...
				{
					outlist = lappend(outlist,
									  online_ExecInitExpr((Expr *) lfirst(l),
												   parent, scalarVarEvalFunc));
				}
				/* Don't fall through to the "common" code below */
				return (ExprState *) outlist;
			}
		...
		default:
			elog(ERROR, "unsupported node type by online sample join: %d",
					(int) nodeTag(node));
			state = NULL;
			break;
	}
