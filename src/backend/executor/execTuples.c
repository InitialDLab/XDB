 ...
 *		ExecSetSlotDescriptor_nopin - set a slot's tuple descriptor without pinning it 
...
ExecDropSingleTupleTableSlot(TupleTableSlot *slot)
{
	...
	Assert(IsA(slot, TupleTableSlot));
	ExecClearTuple(slot);
	if (slot->tts_shouldRelease && slot->tts_tupleDescriptor)
		ReleaseTupleDesc(slot->tts_tupleDescriptor);
	if (slot->tts_values)
		pfree(slot->tts_values);
	if (slot->tts_isnull)
					  ...
					  TupleDesc tupdesc)		/* new tuple descriptor */
{
	...
	slot->tts_shouldRelease = true;
}

void
ExecSetSlotDescriptor_nopin(TupleTableSlot *slot,
							TupleDesc tupdesc) {
	ExecClearTuple(slot);
