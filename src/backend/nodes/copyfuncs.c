...
/* Copy a field that is an array */
#define COPY_INT_ARRAY_FIELD(fldname, n) \
	memcpy(newnode->fldname = palloc(sizeof(int) * (n)),	\
		   from->fldname, sizeof(int) * (n))

#define COPY_ATTRNUMBER_ARRAY_FIELD(fldname, n)	\
	memcpy(newnode->fldname = palloc(sizeof(int) * (n)),	\
			from->fldname, sizeof(AttrNumber) * (n))

#define COPY_OID_ARRAY_FIELD(fldname, n) \
	memcpy(newnode->fldname = palloc(sizeof(Oid) * (n)),	\
		   from->fldname, sizeof(Oid) * (n))

#define COPY_NODE_ARRAY_FIELD(type, fldname, n) \
	do {	\
		uint32 i;	\
		newnode->fldname = (type**) palloc(sizeof(void **) * (n));	\
		for (i = 0; i < (n); ++i) {	\
			newnode->fldname[i] = (type *) copyObject(from->fldname[i]);	\
		}	\
	} while (0)

/* Copy a Numeric field */
#define COPY_NUMERIC_FIELD(fldname)	\
	newnode->fldname = DatumGetNumericCopy(NumericGetDatum(from->fldname))
...
_copyPlannedStmt(const PlannedStmt *from)
{
	...
	COPY_SCALAR_FIELD(nInitSamples);
...
static OnlineSampleJoin *
_copyOnlineSampleJoin(const OnlineSampleJoin *from) {
	OnlineSampleJoin	*newnode = makeNode(OnlineSampleJoin);

	CopyPlanFields((const Plan *) from, (Plan *) newnode);
	
	COPY_SCALAR_FIELD(no);
	COPY_SCALAR_FIELD(nrel);

	COPY_OID_ARRAY_FIELD(relid, from->nrel + 1);
	COPY_OID_ARRAY_FIELD(indexid, from->nrel + 1);
	COPY_OID_ARRAY_FIELD(primary_indexid, from->nrel + 1);
	COPY_SCALAR_FIELD(primaryindex_info);
...
static OnlineAgg *
_copyOnlineAgg(const OnlineAgg *from) {
	OnlineAgg	*newnode = makeNode(OnlineAgg);

	CopyPlanFields((const Plan *) from, (Plan *) newnode);

	COPY_SCALAR_FIELD(numGrpCols);
...
static OnlineAggSum *
_copyOnlineAggSum(const OnlineAggSum *from) {
	OnlineAggSum *newnode = makeNode(OnlineAggSum);

	COPY_NODE_FIELD(arg);
	COPY_SCALAR_FIELD(arg_typeid);
	
	return newnode;
}

static OnlineAggCount *
_copyOnlineAggCount(const OnlineAggCount *from) {
	OnlineAggCount *newnode = makeNode(OnlineAggCount);

	COPY_NODE_FIELD(scale);	
...
static ScanStmt *
_copyScanStmt(const ScanStmt *from) {
  ScanStmt *newnode = makeNode(ScanStmt);
...
copyObject(const void *from)
{
	...
	switch (nodeTag(from))
	{
		...
		case T_OnlineSampleJoin:
			retval = _copyOnlineSampleJoin(from);
			break;
		case T_OnlineAgg:
			retval = _copyOnlineAgg(from);
			break;
		case T_OnlineAggSum:
			retval = _copyOnlineAggSum(from);
			break;
		case T_OnlineAggCount:
			retval = _copyOnlineAggCount(from);
			break;
