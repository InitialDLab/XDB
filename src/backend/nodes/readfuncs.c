...
/* Read a 1D array field */
#define READ_INT_ARRAY_FIELD(fldname, expected_n) \
	token = pg_strtok(&length);	\
	(void) token;	\
	local_node->fldname = (int *) _readArray(NULL, 0, expected_n, 'i')

#define READ_OID_ARRAY_FIELD(fldname, expected_n) \
	token = pg_strtok(&length);	\
	(void) token;	\
	local_node->fldname = (Oid *) _readArray(NULL, 0, expected_n, 'o')

#define READ_NODE_ARRAY_FIELD(type, fldname, expected_n) \
	token = pg_strtoke(&length); \
	(void) token;	\
	local_node->fldname = (type**) _readArray(NULL, 0, expected_n, '\0')
...
static void *
_readArray(char *token, int tok_len, uint32 expected_n, char expected_type)
	...
	if (token) {
		...
		if (type == OTHER_TOKEN) {
			if (tok_len == 0) {
				/* <> */
				if (expected_n != 0) {
					elog(ERROR, "array length does not match expected length %u",
							expected_n);
				}
			}
			else {
			...
			elog(ERROR, "unexpected start of an array");
		}

		token = pg_strtok(&tok_len);
		if (token == NULL) {
			elog(ERROR, "unterminated array structure");
		}
		if (token[0] != 'a') {
			elog(ERROR, "unexpected token: \"%.*s\"", tok_len, token);
		}
	}

	if (tok_len == 2 && token[1] == 'i') {
		/* int array */
		uint32	i, n;
		char	*endptr;
		int		*arr;
		...
		if (expected_type != 'i')
			elog(ERROR, "unexpected integer array");

		token = pg_strtok(&tok_len);
		if (token == NULL)
			elog(ERROR, "unterminated array structure");
		n = (uint32) strtoul(token, &endptr, 10);
		if (endptr != token + tok_len)
			elog(ERROR, "unrecognized array size: \"%.*s\"",
				 tok_len, token);

		if (n == 0)
			elog(ERROR, "zero-sized array");
		arr = palloc(sizeof(int) * n);
		for (i = 0; i < n; ++i) {
			token = pg_strtok(&tok_len);
			if (token == NULL)
				elog(ERROR, "unterminated array structure");
			if (token[0] == ')')
				elog(ERROR, "prematurely teminated array");
			arr[i] = (int) strtol(token, &endptr, 10);
			if (endptr != token + tok_len)
		...
		if (tok_len != 1 || token[0] != ')')
			elog(ERROR, "array with too many elements");
		result = (void *) arr;
	}

	else if (tok_len == 2 && token[2] == 'o') {
		/* oid array */
		uint32	i, n;
		char	*endptr;
		Oid		*arr;
		...
		if (expected_type != 'o')
			elog(ERROR, "unexpected OID array");
		...
		for (i = 0; i < n; ++i) {
			...
			if (token[0] == ')')
				elog(ERROR, "prematurely teminated array");
			arr[i] = (Oid) strtoul(token, &endptr, 10);
			if (endptr != token + tok_len)
		...
		if (expected_type != '\0')
			elog(ERROR, "unexpected node array");
		...
		for (i = 0; i < n; ++i) {
			...
			if (token[0] == ')')
				elog(ERROR, "prematurely teminated array");
			arr[i] = (void *) nodeRead(token, tok_len);
		}
		token = pg_strtok(&tok_len);
		if (token == NULL)
			elog(ERROR, "unterminated array structure");
...
#define MATCH(tokname, namelen) \
  ...
  else if (MATCH("SCANSTMT", 8))
    return_value = _readScanStmt();
	else
	{
