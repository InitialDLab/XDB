...
/* write a 1D array field */
#define WRITE_INT_ARRAY_FIELD(fldname, n)	\
	(appendStringInfo(str, " :" CppAsString(fldname) " "), \
	 _outIntArray(str, node->fldname, (n)))

#define WRITE_ATTRNUMBER_ARRAY_FIELD(fldname, n)	\
	(appendStringInfo(str, " :" CppAsString(fldname) " "), \
	 _outAttrNumberArray(str, node->fldname, (n)))

#define WRITE_OID_ARRAY_FIELD(fldname, n)	\
	(appendStringInfo(str, " :" CppAsString(fldname) " "), \
	 _outOidArray(str, node->fldname, (n)))

#define WRITE_NODE_ARRAY_FIELD(fldname, n)	\
	(appendStringInfo(str, " :" CppAsString(fldname) " "), \
	 _outNodeArray(str, (const void **) node->fldname, (n)))

/* write a numeric field */
#define WRITE_NUMERIC_FIELD(fldname)	\
	(appendStringInfo(str, " :" CppAsString(fldname) " "), \
	 _outNumeric(str, node->fldname))
...
static void _outIntArray(StringInfo str, const int *arr, uint32 n)
__attribute__((unused));

static void
_outIntArray(StringInfo str, const int *arr, uint32 n) {
	uint32 i;
...
static void _outAttrNumberArray(StringInfo str, const AttrNumber *arr, uint32 n)
__attribute__((unused));

static void
_outAttrNumberArray(StringInfo str, const AttrNumber *arr, uint32 n) {
	uint32 i;
	...
	for (i = 0; i < n; ++i) {
		appendStringInfo(str, " %d", (int) arr[i]);
	}
...
static void
_outOidArray(StringInfo str, const Oid *arr, uint32 n) {
	uint32 i;
	...
	appendStringInfo(str, "(ao %u", n);
	
	for (i = 0; i < n; ++i) {
		appendStringInfo(str, " %u", arr[i]);
	}
...
static void
_outNodeArray(StringInfo str, const void **arr, uint32 n) {
	uint32 i;
...
static void _outNumeric(StringInfo str, Numeric x)
	__attribute__((unused));

static void
_outNumeric(StringInfo str, Numeric x) {
	if (x == NULL)
		appendStringInfo(str, "<>");
	else {
		char *sx = (char *) DatumGetPointer(DirectFunctionCall1(numeric_out, 
											NumericGetDatum(x)));
		appendStringInfoString(str, sx);
		pfree(sx);
	}
}
...
_outPlannedStmt(StringInfo str, const PlannedStmt *node)
{
	...
	WRITE_FLOAT_FIELD(confidence, "%.2f");
	WRITE_UINT_FIELD(reportInterval);
	WRITE_UINT_FIELD(nInitSamples);
...
static void
_outOnlineSampleJoin(StringInfo str, const OnlineSampleJoin *node) {
	WRITE_NODE_TYPE("ONLINESAMPLEJOIN");

	_outPlanInfo(str, (const Plan *) node);
	
	WRITE_UINT_FIELD(no);
	WRITE_UINT_FIELD(nrel);

	WRITE_OID_ARRAY_FIELD(relid, node->nrel + 1);
	WRITE_OID_ARRAY_FIELD(indexid, node->nrel + 1);
...
static void
_outOnlineAgg(StringInfo str, const OnlineAgg *node) {
	WRITE_NODE_TYPE("ONLINEAGG");

	_outPlanInfo(str, (const Plan *) node);

	WRITE_INT_FIELD(numGrpCols);
...
static void
_outOnlineAggSum(StringInfo str, const OnlineAggSum *node) {
	WRITE_NODE_TYPE("ONLINEAGGSUM");

	WRITE_NODE_FIELD(arg);
	WRITE_OID_FIELD(arg_typeid);
}

static void
_outOnlineAggCount(StringInfo str, const OnlineAggCount *node) {
	WRITE_NODE_TYPE("ONLINEAGGCOUNT");

	WRITE_NODE_FIELD(scale);
}
...
static void
_outScanStmt(StringInfo str, const ScanStmt *node) {
  WRITE_NODE_TYPE("SCANSTMT");
	...
	else
	{
		...
		switch (nodeTag(obj))
		{
			...
			case T_Limit:
				_outLimit(str, obj);
				break;
			case T_OnlineSampleJoin:
				_outOnlineSampleJoin(str, obj);
				break;
			case T_OnlineAgg:
				_outOnlineAgg(str, obj);
				break;
			case T_OnlineAggSum:
				_outOnlineAggSum(str, obj);
				break;
			case T_OnlineAggCount:
				_outOnlineAggCount(str, obj);
				break;
